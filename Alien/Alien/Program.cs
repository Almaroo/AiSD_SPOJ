...
public static class Program
{
    public static void Main()
    {
        ...
        for (var i = 0; i < testCases; i++)
        {
            ...
            var peopleMax = FastIO.ReadNonNegativeInt();
            
            var trainStops = new int[n];
        
            for (var j = 0; j < n; j++)
            {
                trainStops[j] = FastIO.ReadNonNegativeInt();
            }

            var (peopleMet, stationsVisited) = GetJourneyInfo(trainStops, n, peopleMax);
            stringBuilder.AppendLine($"{peopleMet} {stationsVisited}");
        }

        Console.WriteLine(stringBuilder.ToString());
    }

    private static (int peopleMet, int stationsVisited) GetJourneyInfo(int[] trainStations, int n, int peopleMax)
    {
        var peopleMetDuringLongestJourney = int.MinValue;
        var maxStationsVisited = int.MinValue;
        var nextStationIdx = 1;
        var peopleMet = trainStations[0];
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var first = trainStations[i];

            for (var j = nextStationIdx; j < n; j++)
            {
                if (peopleMet + trainStations[j] > peopleMax) break;
                
                peopleMet += trainStations[j];
                nextStationIdx++;
            }

            var stationsVisited = nextStationIdx - i;
            
            if (stationsVisited > maxStationsVisited)
            {
                maxStationsVisited = stationsVisited;
                peopleMetDuringLongestJourney = peopleMet;
            }
            else if(stationsVisited == maxStationsVisited && peopleMet < peopleMetDuringLongestJourney)
            {
                peopleMetDuringLongestJourney = peopleMet;
            }
        ...
        return (peopleMetDuringLongestJourney, maxStationsVisited);
    }
}
