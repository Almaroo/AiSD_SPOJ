...
public static class Program
{
    ...
    {
        var testCases = FastIO.ReadNonNegativeInt();
        var stringBuilder = new StringBuilder();

        for (var i = 0; i < testCases; i++)
        {
            var n = FastIO.ReadNonNegativeInt();
            var autobotsMax = FastIO.ReadNonNegativeInt();
            
            var planets = new int[n];
        
            for (var j = 0; j < n; j++)
            {
                planets[j] = FastIO.ReadNonNegativeInt();
            }

            var (autobotsMet, planetsDestroyed) = GetPlanetsFightingInfo(planets, n, autobotsMax);
            stringBuilder.AppendLine($"{autobotsMet} {planetsDestroyed}");
        }

        Console.WriteLine(stringBuilder.ToString());
    }

    private static (int autobotsMet, int planetDestroyed) GetPlanetsFightingInfo(int[] planets, int n, int autobotsMax)
    {
        var autobotsDestroyedDuringLongestFight = 0;
        var maxPlanetsDestroyed = 0;
        var nextPlanetIdx = 1;
        var autobotsMet = planets[0];
        var skipped = false;

        for (var i = 0; i < n; i++)
        {
            if (planets[i] > autobotsMax)
            {
                skipped = true;
                continue;
            }
            ...
            var planetsDestroyed = nextPlanetIdx - i > 1 ? nextPlanetIdx - i : 1;

            for (var j = nextPlanetIdx; j < n; j++)
            {
                if (planets[j] + autobotsMet > autobotsMax) break;

                autobotsMet += planets[j];
                planetsDestroyed++;
                nextPlanetIdx++;
            }

            if (planetsDestroyed > maxPlanetsDestroyed)
            {
                maxPlanetsDestroyed = planetsDestroyed;
                autobotsDestroyedDuringLongestFight = autobotsMet;
            }
            else if (planetsDestroyed == maxPlanetsDestroyed && autobotsMet < autobotsDestroyedDuringLongestFight)
                autobotsDestroyedDuringLongestFight = autobotsMet;
        ...
        return (autobotsDestroyedDuringLongestFight, maxPlanetsDestroyed);
    }
}
