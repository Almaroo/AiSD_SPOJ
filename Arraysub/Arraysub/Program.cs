...
public class Test
{
    public static void Main()
    {
        ...
        for (var i = 0; i < n; i++)
        {
            array[i] = FastIO.ReadInt();
        }

        var k = FastIO.ReadInt();
        
        var arrayMaxKRadiusSubarray = GetMaxRadiusKSubarray(array, n, k);
        
        Console.WriteLine(string.Join(' ', arrayMaxKRadiusSubarray));
    }

    private static int[] GetMaxRadiusKSubarray(int[] array, int n, int k)
    {
        ...
        {
            if (maxIdx <= i)
                maxIdx = GetMaxIdx(array, i, k);
            
            if (array[i + k - 1] > array[maxIdx])
                maxIdx = i + k - 1;
    ...
    private static int GetMaxIdx(int[] array, int currentIdx, int k)
    {
        var max = currentIdx;
        
        for (var i = 0; i < k; i++)
        {
            if (array[max] < array[currentIdx + i])
                max = currentIdx + i;
        }
