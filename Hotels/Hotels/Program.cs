...
public static class Program
{
    public static void Main()
    {
        ...
        var max = FastIO.ReadNonNegativeInt();

        var hotels = new int[n];
        
        for (var i = 0; i < n; i++)
        {
            hotels[i] = FastIO.ReadNonNegativeInt();
        }
        
        Console.WriteLine(LargestSubArraySumUnderMax(hotels, n, max));
    }

    private static int LargestSubArraySumUnderMax(int[] array, int n, int max)
    {
        var highest = int.MinValue;
        var first = array[0];
        var slidingSum = first;
        var nextItem = 1;
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            for (var j = nextItem; j < n; j++)
            {
                if (slidingSum + array[j] > max)
                    break;
